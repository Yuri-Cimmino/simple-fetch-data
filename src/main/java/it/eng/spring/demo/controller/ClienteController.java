...
public class ClienteController {
	...
	public ResponseEntity<Object> getSpecificClient(
			@PathVariable @Parameter(description = "Identificativo del cliente", example = "1") Long id,
			@RequestHeader Map<String, String> headers, @RequestHeader(value = "token", required = true) String token) {
	...
	@GetMapping("/cliente/polizze")
	public ResponseEntity<Object> getPolizze() {
		List<PolizzaDTO> polizze = cService.listaPolizze();
		return ResponseEntity.ok(polizze);
	}

	@GetMapping("/cliente/polizze2")
	public ResponseEntity<Object> getPolizze2() {
		PolizzaDTO[] polizze = cService.listaPolizze2();
		return ResponseEntity.ok(polizze);
	}
